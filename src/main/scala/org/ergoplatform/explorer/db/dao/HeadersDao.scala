...
class HeadersDao {
  ...
  def findByParentId(parentId: String): ConnectionIO[Option[Header]] =
    HeadersOps.selectByParentId(parentId).option

  def get(id: String): ConnectionIO[Header] =
    find(id).flatMap {
  ...
  def getByD(d: String): ConnectionIO[Header] =
    findByD(d).flatMap {
      case Some(h) => h.pure[ConnectionIO]
      case None =>
        doobie.free.connection.raiseError(
  ...
  def getLast(limit: Int = 20): ConnectionIO[List[Header]] =
    HeadersOps.selectLast(limit).to[List]

  def getAtHeight(height: Long): ConnectionIO[List[Header]] =
    HeadersOps.selectByHeight(height).to[List]

  def getHeightById(id: String): ConnectionIO[Long] =
    HeadersOps.selectHeight(id).option.flatMap {
      case Some(h) => h.pure[ConnectionIO]
