...
object TransactionsOps extends DaoOps {
  ...
  val tableName: String = "node_transactions_replica"
  ...
  def findAllByBlockId(
    ...
    (fr"SELECT" ++ fieldsFr ++ fr"FROM node_transactions_replica WHERE header_id = $blockId")
      .query[Transaction]

  def countTxsNumbersByBlocksIds(blockIds: NonEmptyList[String]): Query0[(String, Long)] =
    (fr"SELECT header_id, count(*) FROM node_transactions_replica WHERE" ++
  ...
  def getTxsByAddressId(addressId: String, offset: Int, limit: Int)(
    ...
    fr"""
        SELECT DISTINCT t.id, t.header_id, t.coinbase, t.timestamp, t.size
            FROM node_outputs_replica os
            LEFT JOIN node_inputs_replica inp ON inp.box_id = os.box_id
            LEFT JOIN node_transactions_replica t ON (os.tx_id = t.id OR inp.tx_id = t.id)
            LEFT JOIN node_headers_replica h ON  h.id = t.header_id
  ...
  def select(id: String): Query0[Transaction] =
    (fr"SELECT" ++ fieldsFr ++ fr"FROM node_transactions_replica WHERE id = $id")
      .query[Transaction]

  def searchById(substring: String): Query0[String] =
    fr"SELECT id FROM node_transactions_replica WHERE id LIKE ${"%" + substring + "%"}"
      .query[String]

  def countTxsSince(ts: Long): Query0[Long] =
    fr"SELECT count(*) FROM node_transactions_replica WHERE timestamp >= $ts".query[Long]

  def txsHeight(ids: NonEmptyList[String]): Query0[(String, Long)] =
    (fr"SELECT t.id, h.height FROM node_transactions_replica t LEFT JOIN node_headers_replica h ON t.header_id = h.id WHERE" ++
    Fragments.in(fr"t.id", ids)).query[(String, Long)]

  def txHeight(id: String): Query0[Long] =
    fr"SELECT h.height FROM node_transactions_replica t LEFT JOIN node_headers_replica h ON t.header_id = h.id WHERE t.id = $id"
      .query[Long]
}
