...
final case class TransactionsPool(txs: Map[String, ApiTransaction],
  ...
  def getByErgoTree(ergoTree: String): List[ApiTransaction] = indexes
    .get(ergoTree)
    .toList
    .flatten
    .flatMap(get)

  private def extractIndexes(txs: List[ApiTransaction]): Map[String, List[String]] = txs
    .flatMap(tx => tx.outputs.map(_ -> tx.id))
    .foldLeft(Map.empty[String, List[String]]) { case (acc, (out, txId)) =>
